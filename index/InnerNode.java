...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        // TODO(hw2): implement
        int c = numLessThanEqual(key, keys);
        // # of child we want (3 -> leaf3)
        // find right leaf node
        // call get on the right leaf node
        return this.getChild(c).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // find correct leaf
        // LeafNode n = this.get(key);
        //BPlusNode n = getChild(numLessThanEqual(key, keys));
        // insert into the correct leaf
        Optional<Pair<DataBox, Long>> _leaf = getChild(numLessThanEqual(key, keys)).put(key, rid);
        // if leaf splits
        if (!_leaf.equals(Optional.empty())) {
            DataBox leaf_split = _leaf.get().getFirst();
            keys.add(leaf_split); // add split key into inner node
            Collections.sort(keys);
            // add pointers/fix children
            // insert rightsibling of n into index of split key + 1
            children.add(keys.indexOf(leaf_split)+1, _leaf.get().getSecond());
            // n.getRightSibling().get().getPage().getPageNum()
            sync();
        }
        // if inner node splits, d into first, d into second, split key is copied up/deleted from orig inner node
        // redistribute children

        if (keys.size() > 2*order){
            Collections.sort(keys);
            List<DataBox> old_keys;
            List<Long> old_children;
            List<DataBox> new_keys;
            List<Long> new_children;
            DataBox split_key = keys.get(order);

            old_keys = keys.subList(0, order);
            old_children = children.subList(0, order+1);

            new_keys = keys.subList(order+1, 2*order+1);
            new_children = children.subList(order+1, 2*order+2);

            InnerNode _inner = new InnerNode(metadata, bufferManager, new_keys, new_children, treeContext);

            keys = old_keys;
            children = old_children;

            Pair<DataBox, Long> rv = new Pair<DataBox, Long> (split_key, _inner.getPage().getPageNum());
            sync();
            return Optional.of(rv);
        }
        // if inner node doesn't split
...
//        while(data.hasNext() && keys.size() <= 2*metadata.getOrder()) {
            // bulk load rightmost child
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> rightmost = getChild(children.size()-1).bulkLoad(data, fillFactor);
            if (!rightmost.equals(Optional.empty())) {
                DataBox leaf_split = rightmost.get().getFirst();
                keys.add(leaf_split); // add split key into inner node
                Collections.sort(keys);
                children.add(keys.indexOf(leaf_split)+1, rightmost.get().getSecond());
                sync();
            }
            // if inner node is full, split
            if (keys.size() > 2 * order) {
                Collections.sort(keys);
                List<DataBox> old_keys;
                List<Long> old_children;
