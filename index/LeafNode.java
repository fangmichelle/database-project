...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("duplicate entries not allowed");
        }
        ...
        List<DataBox> new_keys = new ArrayList<DataBox>(2*order+1);
        List<RecordId> new_rids = new ArrayList<RecordId>(2*order+1);
        // add new key, rid to old node
        keys.add(key);
        Collections.sort(keys);
        rids.add(rid);
        Collections.sort(rids);
        // split into d, d+1
        // create new node
        // Optional<Long> new_sibling = rightSibling;
        // change siblings
        // add d+1 to new node
        List<DataBox> old_keys = new ArrayList<DataBox>(2*order+1);
        List<RecordId> old_rids = new ArrayList<RecordId>(2*order+1);
        old_keys = keys.subList(0, order);
        old_rids = rids.subList(0, order);
        new_keys = keys.subList(order, 2*order+1);
        new_rids = rids.subList(order, 2*order+1);
        LeafNode n = new LeafNode(metadata, bufferManager, new_keys, new_rids, rightSibling, treeContext);
        rightSibling = Optional.of(n.getPage().getPageNum());
        //sort new keys
        // sort new rids

        // remove last d from old node
        keys = old_keys;
        rids = old_rids;
        Pair<DataBox, Long> rv = new Pair<DataBox, Long> (new_keys.get(0), n.getPage().getPageNum());
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // List<Optional<Pair<DataBox, Long>>> ret= new ArrayList<>();
        if (!data.hasNext()) {
            return Optional.empty();
        }
        int maxFull = (int) Math.ceil(fillFactor * 2 * metadata.getOrder());
        while(data.hasNext()) {
            ...
            Collections.sort(rids);
            if (keys.size() > maxFull) {
                List<DataBox> new_keys = new ArrayList<>();
                List<RecordId> new_rids = new ArrayList<>();
                new_keys.add(keys.remove(keys.size()-1));
                new_rids.add(rids.remove(rids.size()-1));
                Optional<Long> sib = rightSibling;
                LeafNode n = new LeafNode(metadata, bufferManager, new_keys, new_rids, sib, treeContext);
                rightSibling = Optional.of(n.getPage().getPageNum());
                sync();
                Pair pr = new Pair(new_keys.get(0), n.getPage().getPageNum());
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        /* page of siblings */
        Long l = buf.getLong();
        Optional<Long> rightSibling = Optional.of(l);
        if (l == -1) {
