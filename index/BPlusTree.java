...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        // TODO(hw2): implement
        // HOW TO CHECK IF KEY IS IN TREE
    ...
    public Iterator<RecordId> scanAll() {
        ...
        // TODO(hw4_part2): B+ tree locking
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.S);
        LeafNode start = root.getLeftmostLeaf();
        return new BPlusTreeIterator(start);
//        return new BPlusTreeIterator(start, start.scanAll());
    }
...
//        return new BPlusTreeIterator(start, start.scanGreaterEqual(key));
    ...
    public void put(DataBox key, RecordId rid) {
        typecheck(key);
        // TODO(hw2): implement
        // if returns nothing, root didn't split
        Optional<Pair<DataBox, Long>> info = root.put(key, rid);
        if (info.equals(Optional.empty())) {
            return;
        } else {  // root split
            List<DataBox> keys = new ArrayList<DataBox>();
            keys.add(info.get().getFirst());
            List<Long> children = new ArrayList<Long>(2*metadata.getOrder());
            children.add(root.getPage().getPageNum());
            children.add(info.get().getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private LeafNode _start;
        private Iterator<RecordId> _continue;

        // constructor for scanAll
        public BPlusTreeIterator(LeafNode start) {
            _start = start;
            _continue = _start.scanAll();

//            if (!hasNext() || !_continue.hasNext()) {
//                next();
//            }

//            if (! _continue.hasNext()) {
//                LeafNode sib = _start.getRightSibling().get();
//                _start = sib;
//                _continue = _start.scanAll();
//                if (!hasNext()) {
//                    _start = null;
...
//            }
        ...
        // constructor for scanGreaterEqual
        public BPlusTreeIterator(LeafNode start, DataBox key) {
            _start = start;
            _continue = _start.scanGreaterEqual(key);
...
//            }
        ...
        public boolean hasNext() {
            ...
            return _start.getRightSibling().isPresent();
        }
