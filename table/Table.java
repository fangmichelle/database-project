...
public class Table implements BacktrackingIterable<Record> {
    ...
    public Table(String name, Schema schema, HeapFile heapFile, LockContext lockContext) {
        ...
        if (autoToggle && lockContext.saturation(TransactionContext.getTransaction()) >= 0.2
    ...
    public boolean getToggle(Table table) { return table.autoToggle; }
    ...
    public synchronized Record updateRecord(List<DataBox> values, RecordId rid) {
        ...
        double sat = lockContext.saturation(TransactionContext.getTransaction());
        boolean x = autoToggle;
        int y = lockContext.capacity();
        if (autoToggle && lockContext.saturation(TransactionContext.getTransaction()) >= 0.2
                && lockContext.capacity() >= 10) {
            lockContext.escalate(TransactionContext.getTransaction());
        }
