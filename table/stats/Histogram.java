...
public class Histogram {
    ...
    public void buildHistogram(Table table, int attribute) {
        float maximus = Float.MIN_VALUE;
        float minimus = Float.MAX_VALUE;

        //1. first calculate the min and the max values
        // go through iterator and compare/update values (quantization) until find min/max
        BacktrackingIterator<Record> btr = table.iterator();
        btr.markNext();
        while (btr.hasNext()) {
            Record nextVal = btr.next();
            float quant = quantization(nextVal, attribute);
            if (quant > maximus) {
                maximus = quant;
            }
            if (quant < minimus) {
        ...
        //first bucket
        Bucket<Float> firstBucket = new Bucket<Float>(minValue, minValue + width);
        buckets[0] = firstBucket;
        //middle buckets
        for (int i = 1; i < buckets.length - 1; i++) {
            Bucket<Float> buck = new Bucket<Float>(minValue + i*width, minValue + (i+1)*width);
            buckets[i] = buck;
        }
        //last bucket
        Bucket<Float> lastBucket = new Bucket<Float>(minValue + (buckets.length - 2)*width, maxValue);
        ...
        btr.reset();
        while (btr.hasNext()) {
            float NV = quantization(btr.next(), attribute);
            buckets[bucketIndex(NV)].increment(NV);
        }
    ...
    private float [] allEquality(float qvalue) {
        ...
        int BI = bucketIndex(qvalue);
        int i;
        if (qvalue >= maxValue) {
            i = buckets[buckets.length - 1].getDistinctCount();
        } else if (qvalue <= minValue) {
            i = buckets[0].getDistinctCount();
        } else {
            i = buckets[BI].getDistinctCount();
        }
    ...
    private float [] allGreaterThan(float qvalue) {
        ...
        for (int k = 0; k < result.length; k++) {
            if (k == BI) {
                result[k] = (buckets[BI].getEnd()-qvalue)/width;
            } else if (k > BI) {
                result[k] = 1;
            } else if (k < BI) {
                result[k] = 0;
    ...
    private float [] allLessThan(float qvalue) {
        ...
        for (int k = 0; k < result.length; k++) {
            if (k == BI) {
                result[k] = (qvalue-buckets[BI].getStart())/width;
            } else if (k > BI) {
                result[k] = 0;
            } else if (k < BI) {
                result[k] = 1;
