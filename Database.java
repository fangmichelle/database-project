...
//        LockUtil.ensureSufficientLockHeld(getIndexInfoContext(), LockType.X);
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        public Iterator<Record> sortedScan(String tableName, String columnName) {
            ...
            if (getTable(tableName).getToggle(getTable(tableName))
                    && getTableContext(tableName).saturation(TransactionContext.getTransaction()) >= 0.2
                    && getTableContext(tableName).capacity() >= 10) {
                getTableContext(tableName).escalate(TransactionContext.getTransaction());
            }
