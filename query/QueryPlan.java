...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        hs.add(startTableName);

        Map<Set, QueryOperator> singles = new HashMap<Set, QueryOperator>();
        // for startTableName with each of joinTableNames
        finalOperator = minCostSingleAccess(startTableName);
        singles.put(hs, finalOperator);

        for (String s : joinTableNames) {
            hs = new HashSet();
            hs.add(s);
            QueryOperator m = minCostSingleAccess(s);
            singles.put(hs, m);
        }
        ...
        Map<Set, QueryOperator> finalPlan = singles;

        while (finalPlan.size() != 1) {
            finalPlan = minCostJoins(finalPlan, singles);
        }

        finalOperator = minCostOperator(finalPlan);
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int index = -1;
        for (int ind : eligibleIndices) {
            IndexScanOperator iso = new IndexScanOperator(transaction, table, selectColumnNames.get(ind),
                    selectOperators.get(ind), selectDataBoxes.get(ind));
            int estIO = iso.estimateIOCost();
            if (cost > estIO) {
                cost = estIO;
                minOp = iso;
                index = ind;
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        String _leftName = null;
        String _rightName = null;
        QueryOperator op1;
        QueryOperator op2 = null;
        HashSet h = new HashSet();

        //FOR EACH set of tables in prevMap:
        for (Map.Entry<Set, QueryOperator> s : prevMap.entrySet()) {
            ...
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                String [] _left = getJoinLeftColumnNameByIndex(i);
                _leftName = _left[0];
                String [] _right = getJoinRightColumnNameByIndex(i);
                _rightName = _right[0];
                if (s.getKey().contains(_leftName) && !s.getKey().contains(_rightName)) {
                    h = new HashSet();
                    h.add(_rightName);
                    // get operator for rightTable from pass1Map
//                    op2 = pass1Map.get(h);
                    op1 = s.getValue();
                    op2 = pass1Map.get(h);
                } else if (s.getKey().contains(_rightName) && !s.getKey().contains(_leftName)) {
                    h = new HashSet();
                    h.add(_leftName);
                    // get operator for leftTable from pass1Map
//                    op2 = pass1Map.get(h);
                    op1 = pass1Map.get(h);
                    op2 = s.getValue();
                } else {
...
//                op1 = s.getValue();
                QueryOperator q = minCostJoinType(op1, op2, _left[1], _right[1]);
                h.addAll(s.getKey());
                if (!map.containsKey(h)) {
                    map.put(h, q);
                } else if (q.estimateIOCost() < map.get(h).estimateIOCost()) {
                    // check if IO cost is lower than what's already in map, if so, put to replace
                    map.put(h, q);
                }
            }
        }
